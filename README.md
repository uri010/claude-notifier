...
### 권한 요청 배너
![Permission Banner](assets/banner-permission.png)

### 작업 완료 배너
![Stop Banner](assets/banner-stop.png)

### 질문 배너
![Question Banner](assets/banner-question.png)

---

## 특징

- **자동 소멸 없음** — X버튼 / 응답 버튼 / 배너 클릭으로만 닫힘
- **소리 없음** — 어떤 사운드도 재생하지 않음
- **스마트 표시** — 터미널이 이미 포커스 상태이면 배너 생략
- **스택형** — 여러 에이전트 실행 시 배너가 우상단에 겹겹이 쌓임
- **Fail-open** — 앱 미실행 / 타임아웃 시 Claude Code 기본 프롬프트로 폴백
- **tmux 포커스** — 배너 클릭 시 `tmux switch-client` + TTY AppleScript로 정확한 탭 포커스
