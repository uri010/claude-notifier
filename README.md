...
🌐 [소개 페이지](https://uri010.github.io/claude-notifier/) &nbsp;|&nbsp; 📋 [변경 이력 (Changelog)](docs/changelog.html)
...
- **터미널 포커스 시 알림 자동 소멸** — 터미널로 전환하면 작업완료·질문 배너가 자동으로 닫힘 (1.5초 이내). 권한 배너는 사용자가 명시적으로 응답할 때까지 유지
- **정밀 탭·창 감지** — 터미널 앱이 전면이어도 Claude Code가 실행 중인 바로 그 탭/tmux 창이 보일 때만 배너 생략. 다른 탭·창이면 배너 표시 (tmux 같은 세션 다른 창도 올바르게 구분)
...
빌드된 바이너리를 자동으로 다운로드하고 설정합니다. Swift/Xcode 불필요.
...
Claude Code 세션을 재시작하면 훅이 활성화됩니다.
...
curl 설치의 경우 동일한 명령을 재실행하면 최신 버전으로 업데이트됩니다.

소스 빌드의 경우: `git pull && ./scripts/install-launchagent.sh`

## 제거

```bash
# curl 설치 또는 소스 빌드 모두 지원
curl -fsSL https://raw.githubusercontent.com/uri010/claude-notifier/main/scripts/uninstall.sh | bash
```

소스 빌드로 설치한 경우 직접 실행도 가능:
