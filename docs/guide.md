...
| **터미널 포커스 시 자동 소멸** | 터미널로 전환하면 1.5초 이내 모든 배너 자동 닫힘 |
...
다른 앱에 있다가 터미널로 전환하는 순간을 감지해 모든 배너를 자동으로 닫습니다.

```
[카카오톡 사용 중] → 배너 표시
       ↓ 터미널로 전환
[터미널 감지] → clearAll() → 배너 자동 소멸 (최대 1.5초 이내)
```
