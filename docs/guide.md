...
| **터미널 포커스 시 알림 자동 소멸** | 터미널로 전환하면 작업완료·질문 배너가 1.5초 이내 자동 닫힘. 권한 배너는 사용자 응답 전까지 유지 |
...
단순히 터미널 앱이 전면에 있는지만 확인하지 않고, **Claude Code가 실행 중인 바로 그 탭/창**이 보이는지 정확히 확인합니다.

| 상황 | 결과 |
|------|------|
| 터미널이 최전면 + Claude Code가 실행 중인 tmux 창이 보임 | 배너 생략 |
| 터미널이 최전면이지만 다른 탭에 있음 | **배너 표시** |
| 터미널이 최전면이지만 같은 tmux 세션의 다른 창 보는 중 | **배너 표시** |
| 터미널이 최전면이지만 다른 tmux 세션 보는 중 | **배너 표시** |
| 브라우저, IDE 등 다른 앱 | **배너 표시** |

**지원 터미널:**
- **Terminal.app / 터미널** — 선택된 탭의 TTY를 AppleScript로 읽어 정밀 비교
- **iTerm2** — 현재 세션 TTY로 비교
- **Alacritty, Kitty, WezTerm, Hyper** — 단일 창이므로 최전면이면 생략

### 터미널 포커스 시 배너 자동 소멸

ClaudeNotifier 앱은 1.5초 간격으로 최전면 앱을 폴링합니다.  
다른 앱에 있다가 터미널로 전환하는 순간을 감지해 **알림성 배너(작업완료·질문)만** 자동으로 닫습니다.

**권한 요청 배너는 자동으로 닫히지 않습니다** — 터미널로 이동해도 사용자가 명시적으로 Yes/No를 선택할 때까지 유지됩니다. (이전에는 터미널 전환 시 자동 허용(fail-open)되는 버그가 있었습니다.)

```
[카카오톡 사용 중] → 작업완료·질문 배너 표시
       ↓ 터미널로 전환
...
Claude Code가 유휴 상태에서 보내는 "Claude is waiting for your input" 계열 메시지는  
실제 응답이 필요한 질문이 아니므로 배너를 표시하지 않고 무시합니다.
...
- "Claude is waiting" 계열 단순 대기 알림은 필터링하여 미표시
...
프로세스 트리를 10단계까지 거슬러 올라가 실제 pane의 pty(예: `ttys027`)를 찾습니다.

**정밀 tmux 포커스**  
배너 클릭 시 세 단계로 정확한 pane으로 이동합니다:
1. `tmux list-panes -a`로 hook의 TTY를 역조회 → 정확한 `session:window.pane` 좌표 확보  
   (`$TMUX` 기반 `display-message`는 클라이언트 현재 뷰에 따라 잘못된 창을 반환할 수 있어 사용하지 않음)
2. `select-window` + `select-pane` + `switch-client`로 tmux 내부 이동
3. `focusByTTY`로 Terminal.app의 정확한 탭을 최전면으로 이동

**세션 캐시 이중 보장 (allowSession)**  
hook이 45초 타임아웃으로 종료된 뒤 배너 버튼을 클릭해도,  
Swift 앱이 직접 `/tmp/claude-notifier-allowed-<session>` 파일에 도구 이름을 기록합니다.  
다음 hook 호출 시 캐시를 확인해 배너 없이 통과합니다.
...
**터미널 포커스 폴링**  
`NSWorkspace.didActivateApplicationNotification`은 nohup 백그라운드 프로세스에 전달되지 않습니다.  
대신 `RunLoop.main`에 등록된 1.5초 타이머로 `NSWorkspace.shared.frontmostApplication`을 폴링합니다.  
한국어 macOS에서 Terminal.app의 `localizedName`은 `"터미널"`로 반환되므로 이를 명시적으로 포함합니다.
